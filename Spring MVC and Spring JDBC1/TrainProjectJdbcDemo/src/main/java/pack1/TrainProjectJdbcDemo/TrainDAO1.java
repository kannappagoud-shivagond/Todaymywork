		...
		if (null != train) {
         ...
         // Prepared statement in JDBC : insert statement below 			
			jdbcTemplate.update(
					...
					new Object[] {train.getTrainNo(),train.getTrainName(),train.getSource(),
							train.getDistination(),train.getPrice() });
			...
			return (Train) jdbcTemplate.queryForObject(
					...
					new RowMapper<Train>() {
						...
						public Train mapRow(ResultSet resultSet, int arg1)
							...
							train.setDistination(resultSet.getString(4));
							train.setPrice(resultSet.getDouble(5));
							return train;
						}
		});
}
